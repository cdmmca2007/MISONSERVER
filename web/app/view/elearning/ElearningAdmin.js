    ...
    if(!win){
        win=Ext.create('Ext.app.view.component.AppWindow', {
            ...
            buttons :[
            {
                ...
                action: 'save',
                scope:this,
                handler:saveScheduleOnlineExam
            },
            {xtype:'btncancel'}
            ]
        });
...
function saveScheduleOnlineExam(btn){     
     var form = btn.up('window').down('form').getForm();
     if(form.isValid()){
            var obj = form.getValues();
            obj.examdate=new Date(obj.examdate).getTime();
            Ext.Ajax.request({
                url:'onlinexam/schonlnexam.do',
                type:'json',
                headers:{
                ...
                success: function(res){
                    ...
                    if(rec.id!=null) {                       
                    Ext.Msg.alert('Success','Online Exam Scheduled Successfully');
                    }
                    else
...
Ext.define('MyApp.view.elearning.ElearningAdmin' ,{
        ...
        listeners:{
            render: function(component){
                component.getEl().on('click', function(){                    
                   var rec=Ext.getCmp('elearningsetup').getSelectionModel().getSelection()[0];
                   if(rec!=null){
                     scheduleExam(rec.data);
                   }  
                   else
                     Ext.Msg.alert('Warning','Please select Exam from grid to Schedule it.');  
                });
            }
        }
    }
