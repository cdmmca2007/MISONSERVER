...
public class StudentMonthlyFeeDAO {
    ...
    public ExtraFeeModel addOrEditFee(Connection conn,ExtraFeeModel obj) throws ReadableException {
        ...
        try {
           ...
           {               
            ...
            throw new ReadableException(ex.getCause(),ex.getMessage(),"StudentMonthlyFeeDAO", "addoredit");
        }
        return obj;
    ...
    public GenerateFee markPaid(Connection conn, GenerateFee obj) throws ReadableException  {
        ...
        String updatequery="UPDATE generatemonthlyfee SET paid_status=? , paid_on=? , paid_by= ? , paid_amount=? , paymentreceicvedfrom=? , releationwithstud=? , payment_mode=? , cheque_dd_number=? WHERE monthly_fee_id=?";
        int flag=0;
        try {
        if(DaoUtil.executeUpdate(conn, updatequery , new Object[]{1,new Date().getTime(),obj.getPaid_by(),obj.getPaid_amount(),obj.getReceived_from(),obj.getRelationtype(),obj.getPayment_mode(),obj.getCheque_dd_number() ,obj.getMonthly_fee_id()})==1){ 
             conn.commit();
             flag=1;        
   ...
   public void failedwebFeePayment(String rectxt,
            ...
            String insert="INSERT INTO failed_payment_transaction " +
